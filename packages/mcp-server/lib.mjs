/**
 * Token contract query engine — pure functions over the DTCG source files.
 *
 * Reads the same per-theme source sets as build.js, so answers always match
 * what Style Dictionary ships. Token names are DTCG dot-paths
...
/** @returns {Map<string, {name, value, type, tier, description?}>} */
export function loadTheme(theme) {
  ...
  for (const [tier, file] of THEME_SOURCES[theme]) {
    walk(JSON.parse(readFileSync(join(TOKENS_ROOT, file), 'utf8')), [], tier, undefined, out);
  }
...
/**
 * Follow the alias chain from a token to its literal value.
 * Returns { chain: [{name, tier, cssVar}...], value, type }.
 */
export function resolveToken(name, theme) {
...
/**
 * Substring search over token names (separator-insensitive), with optional
 * type/tier filters. Results include the resolved end value.
 */
export function searchTokens({ query = '', type, tier, theme = 'light', limit = 50 } = {}) {
...
/**
 * Given a color value (e.g. sampled from a mockup), rank the closest tokens.
 * Usage-tier tokens win ties: consumers should depend on roles, not scales.
 */
export function suggestToken(color, { theme = 'light', limit = 5 } = {}) {
