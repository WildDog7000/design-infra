...
import { z } from 'zod';
import { resolveToken, searchTokens, suggestToken, THEMES } from './lib.mjs';
...
server.tool(
  ...
  async (args) => asText(searchTokens(args))
);
...
server.tool(
  ...
  async ({ name, theme }) => asText(resolveToken(name, theme))
);
...
server.tool(
  ...
  async ({ color, ...opts }) => asText(suggestToken(color, opts))
);
